          ...
          <div className="grid grid-cols-12 gap-4">
            <div className="col-span-12">
              {nonFeatureData.length === 0 ? (<p>No featured news available.</p>) : (<div className="grid gap-4">
                  {nonFeatureData.map((item) => (<link_1.default href={`/details/${item.category_name}/${item.id}`} key={item.id}>
                      <div className="border-b pb-4" key={item.id}>
                        <div className="card card-side bg-white flex items-center rounded-none">
                          <figure className="relative flex-shrink-0 h-[250px] w-[400px] overflow-hidden rounded-md">
                            <OptimizedNewsImage_1.default imageName={item.title_img || "No image"} altText={`Thumbnail for ${item.news_title}`} heightClass="h-full" widthClass="w-full" priority className="rounded-none object-cover"/>
                          </figure>
