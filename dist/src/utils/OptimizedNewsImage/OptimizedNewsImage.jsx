...
const OptimizedNewsImage = ({ imageName, altText = "News image", priority = false, className = "", hoverEffect = false, heightClass = "", heightStyle, widthClass = "", widthStyle, }) => {
    ...
    return (<div className={`relative overflow-hidden ${heightClass} ${widthClass}`} style={containerStyle}>
      <image_1.default src={hasError ? fallbackImageSrc : resolvedImageSrc} alt={altText} fill priority={priority} className={`${baseClasses} ${className} ${hoverClass}`} sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" quality={75} onError={() => setHasError(true)}/>
    </div>);
};
